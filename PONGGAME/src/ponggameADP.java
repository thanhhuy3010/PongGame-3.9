 ...
 *
 */
public class ponggameADP extends JPanel implements ActionListener, KeyListener {
	...
	private int ballX = 250;// sua 200 thanh 250
	private int ballY = 250;// sua 200 thanh 250
	...
	private int playerOneHeight = 60; // sua 50 thanh 60

	/** Player 2's paddle: position and size */
	private int playerTwoX = 474;// sua 465 thanh 474
		...
		if (playing) {
			...
			if (nextBallLeft < playerOneRight) {
				...
				if (nextBallTop > playerOneBottom || nextBallBottom < playerOneTop) {
					...
					ballDeltaX *= -1; // sua cho bong cham vao thanh ngang se
										// voi lai
				}
			}
			...
			if (nextBallRight > playerTwoLeft) {
				...
				if (nextBallTop > playerTwoBottom || nextBallBottom < playerTwoTop) {
					...
					ballX = 250;// sua 200 thanh 250
					ballY = 250;// sua 200 thanh 250
				} else {

					// If the ball hitting the paddle, it will bounce back
					// FIXME Something wrong here
					ballDeltaX *= -1;// sua cho bong cham vao thanh ngang se voi
										// lai
	...
	public void paintComponent(Graphics g) {
		...
		if (showTitleScreen) {
			...
			g.setColor(Color.YELLOW);// Lam chu Pong Game va Press 'p' to play
										// thanh mau vang
			g.setFont(new Font(Font.DIALOG, Font.BOLD, 36));
			g.drawString("Pong Game", 130, 100);

			// FIXME Wellcome message below show smaller than game title
			g.drawString("Press 'P' to play.", 100, 400); // Chinh lai toa do
															// "Press 'P' to
															// play"
		} else if (playing) {
			...
			// Draw scores
			// TODO Set Blue color
			g.setColor(Color.BLUE); // Chinh hien ra ket qua va diem la mau xanh
									// duong
			g.setFont(new Font(Font.DIALOG, Font.BOLD, 36));
			g.drawString(String.valueOf(playerOneScore), 70, 100);// Chinh lai
																	// toa do
																	// ghi diem
																	// 100 thanh
																	// 70
			...
			if (playerOneScore > playerTwoScore) {
				g.drawString("Player 1 Wins!", 120, 200);// Chinh toa do hien
															// "Player 1 Wins!"
															// chinh giua
			} else {
				g.drawString("Player 2 Wins!", 120, 200);// Chinh toa do hien
															// "Player 2 Wins!"
															// chinh giua
			}
			...
			g.setColor(Color.RED);// Chinh dong "Press 'Space' to start again"
									// la mau do
			g.drawString("Press 'Space' to start again!", 125, 350); // Hien ra
																		// de
																		// biet
																		// bat
																		// dau
